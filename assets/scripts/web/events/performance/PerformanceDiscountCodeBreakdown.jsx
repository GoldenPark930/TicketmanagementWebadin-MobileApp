...
export default class PerformanceDiscountCodeBreakdown extends React.Component {
	getSortedRows(rows_filtered, sort){
		if(sort){
			...
			switch(sort.index){
				case 0:
					rows_filtered = _.orderBy(rows_filtered, (t)=>{return t.discountCode}, dir)
					break
				case 1:
	...
	getClipboardText(rows_filtered){
		let ret = ''
		ret += 'Discount Code' + '\t' + 'Ticket Type' + '\t' + 'Price (excl. Fees)' + '\t' + 'No of Sales' + '\t' + 'Revenue' + '\n'
	...
	render() {
		...
		return (
			<div>
				<JSONDatatable 
					type={TYPE_FROM_URL}
					data={{url: `/api/events/${event.id}/relationships/performance/`, param:{'section': 'discount_code_breakdown'}, node: 'data.discount_code_breakdown.*'}}
					...
					usePagination={false}
					autoRefresh={20 * 1000}
					loadingBarTitle={'Hold tight! We\'re getting your event\'s sales by discount code...'}
				>
					<div ref={DATATABLE}/> 
				</JSONDatatable>
			</div>				
