...
; (function ($, window, document, undefined) {
    ...
    var pluginName = 'mediumInsert',
        ...
        defaults = {
            ...
            oembedProxy: '//medium.iframe.ly/api/oembed?iframe=1&api_key=6e0288d71094809ca959d4',
            captions: true,
    ...
    /**
     * Convert video url to embed string
     *
     * @param {string} url
     * @return {string} html tag
     * @author Masao [9/27/2016]
     */

    Embeds.prototype.convertVideo = function(html){
        var pattern1 = /(?:http?s?:\/\/)?(?:www\.)?(?:vimeo\.com)\/?(.+)/g;
        var pattern2 = /(?:http?s?:\/\/)?(?:www\.)?(?:youtube\.com|youtu\.be|y2u\.be)\/(?:watch\?v=)?([^\&]+)(?:\&.+)?/g;

        var div_prefix = '<div class="video-container">';
        var div_subfix = '</div>';

        if(pattern1.test(html)){
          var replacement = '<iframe src="//player.vimeo.com/video/$1" frameborder="0" allowfullscreen="true" webkitallowfullscreen="true" mozallowfullscreen="true"></iframe>';
          var html = div_prefix + html.replace(pattern1, replacement) + div_subfix;
          return html;
        }

        if(pattern2.test(html)){
          var replacement = '<iframe src="//www.youtube.com/embed/$1" frameborder="0" allowfullscreen="true" webkitallowfullscreen="true" mozallowfullscreen="true"></iframe>';
          var html = div_prefix + html.replace(pattern2, replacement) + div_subfix;
          return html;
        }
        return html;
    }
    ...
    Embeds.prototype.oembed = function (url, pasted) {        
        ...
        var html = this.convertVideo(url);        
        if (pasted) {
...
; (function ($, window, document, Util, undefined) {
    ...
    Images.prototype.add = function () {
        var that = this,
            ...
            fileUploadOptions = {
                add: function (e, data) {
                    that.options.fileUploadOptions.add(e, data);
    ...
    Images.prototype.uploadDone = function (e, data) {
        $.proxy(this, 'showImage', data.context.fileURL, data)(); //data.result.files[0].url
